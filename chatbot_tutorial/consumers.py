...
def chat_send(message):
    ...
    count = Chat.objects.all().count()
    user = "user_"+ str(count-1)
    chat = Chat.objects.get(user=user)
    stupid = chat.stupid
    fat = chat.fat
    dumb = chat.dumb

    stupid = stupid + message['stupid']
    fat = fat + message['fat']
    dumb = dumb + message['dumb']
    
    Chat.objects.filter(user=user).update(stupid=stupid, fat=fat, dumb=dumb)
...
#     chat = Chat.objects.get(user='user_1')
#     stupid = chat.stupid
#     fat = chat.fat
#     dumb = chat.dumb
...
#     fat = fat + message['fat']
#     dumb = dumb + message['dumb']
#     # message = json.loads(stupid, fat, dumb)
#     Chat.objects.update(stupid=stupid, fat=fat, dumb=dumb)